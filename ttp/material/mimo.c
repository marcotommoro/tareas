...
void algoritmo1( int n, double *x, double *b, double nrm );
...
/**
 *     Algoritmo1 del enunciado de la práctica en el que se trabajo con el cuadrado de la norma y se va calculando según 
 *     se sugiere en la misma memoria.
 *          
 *     n   (int)           (input) Tamaño del subvector de x sobre el que se está trabajando, x(0:n-1). Las componentes x(n:lda) se han calculado ya.
 *     x   (double*)       (input/output) Puntero al primer elemento del vector x (completo).
 *     b   (double*)       (input) Puntero al primer elemento del vector b (completo).
 *     nrm (double)        (input) Norma calculada hasta el momento de la llamada a este algoritmo.
 *
 *     Variables externas. Las siguientes variables forman parte del problema, son accedidas por el algoritmo pero no se modifican. 
 *     t   (int)           (input) Tamaño del conjunto discreto I de símbolos.
 *     I   (double*)       (input) Puntero al primer elemento del vector donde se encuentran los elementos del conjunto discreto I.
 *     R   (double*)       (input) Puntero al primer elemento de la matriz triangular superior R de tamaño lda x n.
 *     lda (int)           (input) (Leading dimension) Número de filas de la matriz R. Coincide con el tamaño de los vectores x y b en la primera llamada a la función.
 *
 *     Variables externas. Las siguientes variables forman parte del problema, en realidad, son la solución:
 *     minimo (int)        (input/output) Valor mínimo.
 *     sol    (int)        (input/output) Vector solución.
 *
 */
void algoritmo1( int n, double *x, double *b, double nrm ) {
  int k, inc = 1;
